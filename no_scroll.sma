...
public plugin_init()
{
	register_plugin("No Scroll", "1.1", "Empower")
		...
		else
		{
			if(!g_bUseScroll[id])
			{
				...
				if(fGameTime - g_fDuckTime[id] < 0.02) // 0.015, 0.02, 0.03
				{
					...
					//client_cmd(id, "spk ^"%s^"", "buttons/bell1.wav")
					
					new sName[MAX_NAME_LENGTH]
					get_user_name(id, sName, charsmax(sName))
					
					client_print_color(0, id, "^1El jugador ^3%s^1 intentÃ³ hacer uso del scroll.", sName)
				}
			}
